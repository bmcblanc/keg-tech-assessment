...
export const addGoatFacts = (facts = []) => {
  ...
  document.querySelector('#no-goat-fact-found').className = (facts.length > 0) ? 'd-none' : 'd-block'

  facts.forEach((fact, idx) => {
    const goatFactItem = document.createElement('li');
    goatFactItem.classList.add('list-group-item', (idx % 2 === 0 ? 'list-group-item-info' : 'list-group-item-light'))
